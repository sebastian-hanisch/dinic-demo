...
def random_bipartite(n, degree, seed):
    """Zufälliges bipartites Einheitsnetz (nur für die Auswertung): n Fahrzeuge, n Aufträge, jedes Fahrzeug mit `degree` verschiedenen Aufträgen verbunden."""
    rng = SplitMix64(seed)
    names, pos = ["S", "T"], [(50, 96), (50, 4)]
    xs = _row_x(n)
    names += [f"F{i + 1}" for i in range(n)] + [f"A{i + 1}" for i in range(n)]
    pos += [(xs[i], 70) for i in range(n)] + [(xs[i], 30) for i in range(n)]
    arcs = [(0, 2 + i, 1, 0) for i in range(n)]
    for i in range(n):
        chosen = []
        while len(chosen) < min(degree, n):
            j = rng.below(n)
            if j not in chosen:
                chosen.append(j)
        arcs += [(2 + i, 2 + n + j, 1, 0) for j in sorted(chosen)]
